...
areas.uso <- censo |> 
  ...
  mutate(periodo = case_when(ano_construcao < 2004 ~ "antigo",
...
censo.areas <- areas.uso |> 
  ...
  mutate(area_total = total_NA,
         ...
         area_residencial_antigo_percent = ifelse(ano == 2022, residencial_antigo + residencial_pre, residencial_antigo) / area_lote_absoluto,
         area_lote = area_lote_absoluto / area_total) |> 
  left_join(censo |> 
   ...
   mutate(area_eixo = ifelse(is.na(area_eixo_total), 0, area_eixo_total / area_total),
          area_eixo_lote = replace_na(area_eixo_total, 0) / (area_lote_absoluto)) |> 
  select(id_setor, ano, moradores, domicilios, area = area_total, area_lote, area_residencial, area_residencial_antigo_percent, area_residencial_novo_percent, area_eixo, area_eixo_lote, geometry)
...
df.censo <- censo.distancias |>
  ...
  select(id_setor, ano, distancia, grupo, moradores, domicilios, starts_with("densidade"), 
         area_residencial, area_residencial_novo_percent, area_residencial_antigo_percent)
...
data <- df.censo |>
  ...
  mutate(morador_dom = moradores / domicilios)
...
modelsummary(list(
  ...
  "(D)" = lm(moradores ~ factor(grupo) * factor(ano) * area_residencial_novo_percent, data = data),
  "(E)" = lm(morador_dom ~ factor(grupo) * factor(ano) * area_residencial_novo_percent, data = data),
  "(F)" = lm(domicilios ~ factor(grupo) * factor(ano) * area_residencial_novo_percent, data = data)),
...
gg <- df.censo |> 
  ...
  (\(df, start_roll = 250, end_roll = 35, step = 5) {
    seq(start_roll, end_roll, by = -step) |>
      purrr::map_dfr(\(banda) {
        df |>
          filter(abs(distancia) <= banda) |>
          lm(densidade ~ factor(grupo) * factor(ano) * area_residencial_novo_percent, data = _) |>
