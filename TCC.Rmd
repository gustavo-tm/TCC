...
data |> 
  ...
  summarize(moradores = sum(moradores)) |> 
...
(df.censo |> 
  ...
  mutate(id_setor = str_sub(id_setor, 1, 15)) |> 
  group_by(id_setor) |> 
  filter(grupo != "descartado", n() == 2, abs(area / sum(area) - .5) < .001, abs(distancia / sum(distancia) - .5) < .001) |> 
...
bind_rows(
  ...
  labs(x = "", y = "Coeficiente do DiD (Intervalos de 95% de confiança)") +
  theme_bw() +
  theme(legend.position = "bottom")

ggsave("tex/figuras/event-study.pdf", width = 7, height = 7)
```

```{r "analise/delta-IPTU"}
df.IPTU |> 
  drop_na() |> 
  filter(abs(distancia) <= 50, ano_construcao > 2002, ano_construcao <= 2022, grupo != "descartado") |> 
  mutate(ano_construcao = as.integer(ano_construcao)) |> 
  group_by(grupo, ano_construcao) |> 
  summarize(lotes = n()) |> 
  group_by(ano_construcao) |>
  mutate(lotes = ifelse(grupo == "controle", -lotes, lotes),
         soma = sum(lotes),
         grupo = factor(grupo, levels = c("tratamento", "controle"))) |> 
  ...
  scale_colour_manual("", values = c("diff" = "black"), labels = c("diff" = "Diferença entre os grupos")) +
  scale_fill_manual("Grupo", values = c("tratamento" = "#E8B92FFF", "controle" = "#89973DFF")) +
  labs(x = "Ano da construção", y = "Número de lotes edificados") +
  theme_minimal()
...
df.IPTU |> 
  drop_na() |> 
  filter(abs(distancia + 5) <= 50, ano_construcao > 2002, ano_construcao <= 2022, grupo != "descartado") |> 
  mutate(ano_construcao = as.integer(ano_construcao)) |> 
  group_by(grupo, ano_construcao) |> 
  summarize(unidades = sum(unidades)) |> 
  group_by(ano_construcao) |>
  mutate(unidades = ifelse(grupo == "controle", -unidades, unidades),
         soma = sum(unidades),
         grupo = factor(grupo, levels = c("tratamento", "controle"))) |> 
  ggplot(aes(x = ano_construcao)) +
  ...
  scale_colour_manual("", values = c("diff" = "black"), labels = c("diff" = "Diferença entre os grupos")) +
  scale_fill_manual("Grupo", values = c("tratamento" = "#E8B92FFF", "controle" = "#89973DFF")) +
  labs(x = "Ano da construção", y = "Número de unidades construídas") +
  theme_minimal()
ggsave("tex/figuras/IPTU-delta-lotes.pdf", width = 8, height = 3.5)
...
(df.IPTU |>
  ...
  (\(df) rdplot(as.numeric(df$densidade_construtiva), df$distancia,
     ...
     labs(title = "",  x = "Distância",y = "Densidade Construtiva") +
     theme_minimal())(df = _) +
...
df.IPTU |>
  ...
  (\(df) rdplot(as.numeric(df$densidade_habitacional), df$distancia,
                binselect = "qs", p = 1, col.dots = "black", col.lines = "black")$rdplot + 
     labs(title = "",  x = "Distância",y = "Densidade Habitacional") +
     theme_minimal())(df = _) +
...
df.IPTU |>
  ...
  (\(df) rdplot(as.numeric(df$pavimentos), df$distancia,
                binselect = "qs", p = 1, col.dots = "black", col.lines = "black")$rdplot + 
     labs(title = "",  x = "Distância",y = "Pavimentos") +
     theme_minimal())(df = _) +
...
df.IPTU |>
  ...
  (\(df) rdplot(as.numeric(df$residencial), df$distancia,
                binselect = "qs", p = 1, col.dots = "black", col.lines = "black")$rdplot + 
     labs(title = "",  x = "Distância",y = "Residencial") +
     theme_minimal())(df = _)) |> 
