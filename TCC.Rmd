...
areas.uso <- censo |> 
  ...
  mutate(periodo = case_when(ano_construcao < 2002 ~ "antigo",
                             ano_construcao <= 2010 ~ "pre",
                             ano_construcao > 2014 ~ "pos")) |> 
...
censo.areas <- areas.uso |> 
  ...
  mutate(area = total_NA,
         area_lote_absoluto = outros_antigo + outros_pos + outros_pre + outros_NA + residencial_antigo + residencial_pos + residencial_pre,
         area_residencial = (residencial_antigo + residencial_pos + residencial_pre) / area_lote_absoluto,
         area_residencial_novo_percent = ifelse(ano == 2022, residencial_pos, residencial_pre) / area_lote_absoluto,
         teste = residencial_pos / area_lote_absoluto,
  ...
  select(id_setor, ano, moradores, domicilios, area, area_lote, area_residencial, area_residencial_novo_percent, teste, area_eixo, area_eixo_lote, geometry)
...
df.censo <- censo.distancias |>
  mutate(grupo = case_when(posicao_centroide == "eixo" & as.numeric(area_eixo_lote) > .9 ~ "tratamento",
                           posicao_centroide == "fora" & as.numeric(area_eixo_lote) < .05 ~ "controle",
                           TRUE ~ "descartado"),
  ...
  select(id_setor, ano, distancia, grupo, area, moradores, domicilios, starts_with("densidade"), 
         area_residencial, area_residencial_novo_percent, teste)
...
data |> filter(grupo == "tratamento", ano == 2010) |> summarize(med = mean(densidade))

modelsummary(list(
  "(A)" = lm(densidade ~ factor(grupo) * factor(ano), data = data),
  "(B)" = lm(densidade ~ factor(grupo) * factor(ano) * teste, data = data)),
  ...
  coef_map = c("(Intercept)" = "Intercepto",
               ...
               "teste" = "Q (Percentual residencial novo)",
               "factor(grupo)tratamento:factor(ano)2022" = "T x P",
               "factor(grupo)tratamento:teste" = "T x Q",
               "factor(ano)2022:teste" = "P x Q",
               "factor(grupo)tratamento:factor(ano)2022:teste" = "T x P x Q"),
  gof_omit = "AIC|BIC|Log.Lik.|RMSE")
  tab_caption("Resultado diff-in-diff do Censo, para setores censitários em um raio de 100m dos eixos") |> 
  gtsave("tex/tabelas/did-censo.tex")

modelsummary(list(
  "(A)" = lm(densidade ~ factor(grupo) * factor(ano), data = data),
  "(B)" = lm(densidade ~ factor(grupo) * factor(ano) * area_residencial_novo_percent, data = data)),
  stars = TRUE, statistic = NULL,
  coef_map = c("(Intercept)" = "Intercepto",
               "factor(grupo)tratamento" = "T (Grupo de tratamento)",
               "factor(ano)2022" = "P (Período pós PDE)",
               "area_residencial_novo_percent" = "Q (Percentual residencial novo)",
               "factor(grupo)tratamento:factor(ano)2022" = "T x P",
               "factor(grupo)tratamento:area_residencial_novo_percent" = "T x Q",
               "factor(ano)2022:area_residencial_novo_percent" = "P x Q",
               "factor(grupo)tratamento:factor(ano)2022:area_residencial_novo_percent" = "T x P x Q"),
  gof_omit = "AIC|BIC|Log.Lik.|RMSE")
  
modelsummary(list(
  "(A)" = lm(densidade ~ factor(grupo) * factor(ano), data = data),
  "(B)" = lm(densidade ~ factor(grupo) * factor(ano) * teste, data = data),
  "(B)" = lm(moradores ~ factor(grupo) * factor(ano) * teste, data = data),
  "(B)" = lm(domicilios ~ factor(grupo) * factor(ano) * teste, data = data)),
  stars = TRUE, statistic = NULL)
...
data <- df.censo |>
  ...
  group_by(id_setor) |> 
  filter(grupo != "descartado", abs(distancia) <= banda, n() == 2, abs(area / sum(area) - .5) < .01, abs(distancia / sum(distancia) - .5) < .01)


modelsummary(list(
  "(A)" = feols(densidade ~ factor(grupo) * factor(ano) | id_setor, data = data),
  "(B)" = feols(densidade ~ factor(grupo) * factor(ano) * area_residencial_novo_percent | id_setor, data = data)),
  stars = TRUE, statistic = NULL)
...
#cenário2
(beta_0 + beta_2 + q*beta_3 + q*beta_6)  * 10^6
(beta_0 + q*beta_3)  * 10^6

#cenário3
(beta_0 + beta_1 + beta_2 + beta_4)  * 10^6
(beta_0 + beta_1)  * 10^6

#cenário4
(beta_0 + beta_1 + beta_2 + q*beta_3 + beta_4 + q*beta_5 + q*beta_6 + q*beta_7)  * 10^6
(beta_0 + beta_1 + q*beta_3 + q*beta_5)  * 10^6
...
data <- df.censo |>
  ...
  mutate(id_setor = str_sub(id_setor, 1, 15)) |> 
  group_by(id_setor) |> 
  filter(n() == 2, n() == 2, abs(area / sum(area) - .5) < .01, abs(distancia / sum(distancia) - .5) < .01)

feols(densidade ~ factor(grupo) * factor(ano) | id_setor, data = data) |> 
...
gg <- df.censo |> 
  ...
  mutate(id_setor = str_sub(id_setor, 1, 15)) |> 
  group_by(id_setor) |> 
  filter(grupo != "descartado", n() == 2, abs(area / sum(area) - .5) < .01, abs(distancia / sum(distancia) - .5) < .01) |> 
  (\(df, start_roll = 150, end_roll = 50, step = 5) {
    seq(start_roll, end_roll, by = -step) |>
      purrr::map_dfr(\(banda) {
        df |>
          filter(abs(distancia) <= banda) |>
          feols(densidade ~ factor(grupo) * factor(ano) * area_residencial_novo_percent| id_setor, data = _) |>
...
gg
ggsave("tex/figuras/did-censo-bandas.pdf", width = 8, height = 4)


data |> 
  filter(grupo != "descartado") |> 
  (\(df, start_roll = 200, end_roll = 20, step = 5) {
    seq(start_roll, end_roll, by = -step) |>
      purrr::map_dfr(\(banda) {
        df |>
          filter(abs(distancia) <= banda) |>
  ...
  mutate(term = case_when(term == "factor(grupo)tratamento" ~ "Tratamento",
                          term == "factor(grupo)tratamento:factor(ano)2022" ~ "Tratamento x Pós PDE",
                          term == "factor(ano)2022" ~ "Pós PDE",
                          TRUE ~ NA)) |> 
  drop_na() |> 
  ggplot(aes(x = banda, y = estimate, colour = term, fill = term)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
...
extrair.coefs <- functias.numeric()extrair.coefs <- function(regressao){
...
bind_rows(
  ...
  feols(residencial ~ i(factor(ano_construcao), factor(grupo), ref = 2014), 
        data = data) |> 
    extrair.coefs() |> mutate(variavel = "Residencial", residencial = T),
  ...
  mutate(ano = str_sub(nome, 27,28), 
         ...
         se = as.numeric(se)) |> 
  bind_rows(data.frame(ano = "14", coef = 0, se = 0, residencial = 1:8<=4, 
                       variavel = c("Densidade Habitacional", "Pavimentos", "Densidade Construtiva", "Residencial"))) |> 
  ...
  labs(x = "", y = "Coeficiente do DiD", caption = "Intervalos de 95% de confiança") +
  theme_bw() +
  theme(legend.position = "bottom")

ggsave("tex/figuras/event-study.pdf", width = 5, height = 5.5)
```
```{r}
df.IPTU |> 
  filter(abs(distancia) <= 100, ano_construcao <= 2014) |> 
  group_by(grupo) |> 
  summarize(unidades = sum(unidades))

df.IPTU |> 
  filter(abs(distancia) <= 100, ano_construcao > 2014, uso_predominante == "residencial") |> 
  group_by(grupo) |> 
  summarize(unidades = sum(unidades))
```
