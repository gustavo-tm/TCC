...
% Por fim, ao ligar os pontos e observar o impacto da regulação sobre a densidade populacional nos EETUs, as evidências sugerem que a política implementada pelo PDE de fato resultou em um aumento na ocupação populacional dessas áreas. Por outro lado, mesmo que o PDE tenha exercido um efeito positivo no adensamento populacional, o resultado depois de 10 anos de vigência do plano ainda não é suficiente para se observar uma grande parcela da população começando a habitar os eixos. Ainda assim, o PDE, por meio dos instrumentos de regulação conseguiu direcionar o adensamento populacional, ainda que com intensidade moderada, para as zonas no entorno do transporte público de alta capacidade, que era seu objetivo principal.  
...
\section{Principais limitações}

% A principal limitação da pesquisa é o nível de observação dos dados do Censo. Os dados do Censo geralmente são tratados como microdados, no qual cada observação é uma pessoa. Todavia, como esta pesquisa lida com dados georreferenciados, é necessário utilizar o nível de setor censitário, que é a escala mais desagregada de dado disponível do Censo. O problema do setor censitário é que ele agrega centenas de entrevistados em um grupo, o que inevitavelmente diminui a granularidade da análise.

% Este problema no caso desta pesquisa é grave, visto que os setores censitários muitas vezes apresentam uma grande extensão geográfica, então foi comum que um setor tivesse parte de sua área em região que recebeu tratamento e outra na região de controle. Na análise conduzida na Seção \ref{sec:perg3}, o número de habitantes dos setores censitários que precisaram ser excluídos da análise este motivo em 2022 foi de 504.517. O número de habitantes que restaram no grupo de tratamento, ou seja, o setor censitário estava completamente envolvido pela região de eixo, foi 680.616. Este é um dos principais fatores de perda do poder dos testes, já que o tamanho da amostra se reduz bastante.

% Ademais, a malha do setor censitário não se mantém igual entre um ano e outro, dificultado o acompanhamento dos setores censitários no tempo. Na análise conduzida, apenas cerca de 10\% dos setores censitários na área de interesse não mudaram de forma e puderam ser acompanhados entre 2010 e 2022.
...
% Como as mudanças na regulação não necessariamente permitem um lucro maior e, em algumas casos pode reduzi-lo, não há forças que aumentam oferta de habitação, o que caminha na direção oposta aos objetivos do PDE.

% Portanto, a formulação do próximo Plano Diretor Estratégico deve considerar ajustes que alinhem incentivos econômicos e regulatórios aos objetivos de adensamento sustentável, garantindo que as áreas de eixos sejam ocupadas de maneira eficiente e contribuam para uma cidade mais inclusiva e conectada.