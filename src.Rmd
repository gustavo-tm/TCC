...
```{r}
censo.did <- censo |> 
  mutate(intersec = st_intersects(centroid, eixos.contorno |> st_buffer(100)) |> as.logical() |> replace_na(FALSE)) |> 
  filter(intersec)
...
IPTU.did |> 
  ...
  feols(pavimentos ~ i(factor(ano_construcao), factor(grupo), ref = 2014), data = _) |> 
  summary() |> 
  (\(summ) summ[["coefficients"]] |> bind_rows() |> pivot_longer(everything()))() |> 
  filter(endsWith(name, "Tratamento")) |> 
  mutate(ano = str_sub(name, 25,28)) |> 
  bind_rows(data.frame(ano = "2014", value = 0, name = "")) |> 
  ggplot() +
  geom_point(aes(x = ano, y = value))
