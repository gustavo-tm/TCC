...
censo.areas <- censo |> 
  ...
  rename(geometry_lote = geometry) |> 
  group_by(id_setor, tipo_lote, uso_predominante) |> 
  summarize(area_cortada = st_union(geometry_lote) |> st_area() |> as.numeric()) |> 
  mutate(uso_predominante = replace_na(uso_predominante, ""), 
         uso = paste("area", tipo_lote, uso_predominante, sep = "_")) |> 
  ungroup() |> st_drop_geometry() |> 
  group_by(id_setor, uso) |> 
  summarize(area = sum(area_cortada)) |> 
  # pivot_wider(id_cols = id_setor, names_from = uso, values_from = area) |>
  # mutate(across(everything(), ~ replace_na(.x, 0))) |> 
  bind_rows(censo |> 
              st_drop_geometry() |> 
              select(id_setor, area) |> 
              mutate(area = as.numeric(area)) |> 
              pivot_longer(area, names_to = "uso", values_to = "area"))


saveRDS(censo.areas, "rdata/censo-areas.rds")

# DIMINUIR BUFFER DO CENSO E AUMENTAR DO IPTU (200M?) 

censo.areas |> 
  group_by(uso) |> 
  summarize(area = sum(area)) |> 
  separate_wider_delim(uso, delim = "_", names_sep = "_", too_few = "align_start") |> 
  mutate(uso_1 = "Área do setor \n censitário",
         uso_2 = case_when(uso_2 == "F" ~ "Fiscal",
                           uso_2 == "M" ~ "Espaço Livre",
                           uso_2 == "V" ~ "Vias de Acesso",
                           TRUE ~ "Não lote"),
         uso_3 = case_when(uso_3 == "" ~ "outros",
                           is.na(uso_3)~ "outros",
                           TRUE ~ uso_3)) |>
  make_long(uso_1, uso_2, uso_3, value = area) |> 
  ggplot(aes(x = x, 
               next_x = next_x, 
               node = node, 
               next_node = next_node,
             ...
             value = value,
             label = node)) +
  geom_sankey(flow.alpha = 0.75, colour = "black") +
  geom_sankey_label(size = 3.5, color = "black", fill = "white", alpha = .9) +
  theme_sankey() +
  scale_fill_viridis_d(option ="G") +
  scale_x_discrete(name = "", position = "top", labels = c("uso_1" = "Setor censitário",
                                                           "uso_2" = "Tipo de lote",
                                                           "uso_3" = "Qualificação no IPTU")) +
...
```{r}
# Função auxiliar para aplicar modelos e extrair coeficientes
extrair_coeficientes <- function(data, formula, variavel_nome, residencial) {
  data %>%
    feols(formula, data = .) %>%
    summary() %>%
    (\(summ) {
      data.frame(
        nome = names(summ$coefficients),
        coef = unname(summ$coefficients),
        se = unname(summ$se)
      )
    })() %>%
    mutate(variavel = variavel_nome, residencial = residencial)
}

# Filtragem inicial e aplicação dos modelos
dados_modelos <- IPTU.did %>%
  filter(grupo != "Descartado") %>% 
  drop_na() %>%
  bind_rows(
    extrair_coeficientes(., densidade_construtiva ~ i(factor(ano_construcao), factor(grupo), ref = 2014), "Densidade Construtiva", residencial = TRUE),
    extrair_coeficientes(., densidade_habitacional ~ i(factor(ano_construcao), factor(grupo), ref = 2014), "Densidade Habitacional", residencial = TRUE),
    extrair_coeficientes(., pavimentos ~ i(factor(ano_construcao), factor(grupo), ref = 2014), "Pavimentos", residencial = TRUE),
    extrair_coeficientes(., densidade_construtiva ~ i(factor(ano_construcao), factor(grupo), ref = 2014), "Densidade Construtiva", residencial = FALSE),
    extrair_coeficientes(., densidade_habitacional ~ i(factor(ano_construcao), factor(grupo), ref = 2014), "Densidade Habitacional", residencial = FALSE),
    extrair_coeficientes(., pavimentos ~ i(factor(ano_construcao), factor(grupo), ref = 2014), "Pavimentos", residencial = FALSE)
  )

# Processamento final e ajuste para visualização
dados_plot <- dados_modelos %>%
  filter(endsWith(nome, "Tratamento")) %>%
  mutate(
    ano = str_sub(nome, 27, 28),
    coef = as.numeric(coef),
    se = as.numeric(se)
  ) %>%
  bind_rows(data.frame(
    ano = "14", coef = 0, se = 0, nome = "", 
    variavel = c("Densidade Habitacional", "Pavimentos", "Densidade Construtiva"),
    residencial = c(TRUE, TRUE, TRUE, FALSE, FALSE, FALSE)
  ))

# Plotagem
ggplot(dados_plot, aes(x = ano, y = coef, colour = residencial)) +
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = "14", linetype = "dotted") +
  geom_point() +
  geom_errorbar(aes(ymin = coef - 1.96 * se, ymax = coef + 1.96 * se)) +
  facet_grid(rows = vars(variavel), scales = "free_y") +
  theme_minimal() +
  scale_colour_viridis_d(option = "G", begin = .25, end = .75)
```
