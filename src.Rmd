...
ggsave("output/mapa-lotes.pdf", gg, height = 10, width = 10)
ggsave("output/mapa-lotes-metro.pdf", height = 10, width = 10,
       plot = gg +   
         geom_point(data = metro, aes(x = x, y = y), 
                    size = 8, shape = 15, colour = "white", fill = "white") +
         geom_image(data = metro, aes(x = x, y = y, image = "output/icone_metro.png"), size = .025) +
...
```{r "analise/did-IPTU"}
data <- df.IPTU |> 
  filter(abs(distancia) <= 150, grupo != "descartado") |> 
  mutate(residencial = uso_predominante == "residencial")

modelsummary(list(
  "densidade_construtiva" = lm(densidade_construtiva ~ factor(grupo) * factor(posPDE), data = data),
  "densidade_habitacional" = lm(densidade_habitacional ~ factor(grupo) * factor(posPDE), data = data),
  "pavimentos" = lm(pavimentos ~ factor(grupo) * factor(posPDE), data = data), 
...
ggsave("output/event-study.pdf", width = 8, height = 6)
```
...
ggsave("output/rdd-balanceamento-censo.pdf", width = 6, height = 4)
...
ggsave("output/rdd-balanceamento-censo-filtrado.pdf", width = 6, height = 4)
